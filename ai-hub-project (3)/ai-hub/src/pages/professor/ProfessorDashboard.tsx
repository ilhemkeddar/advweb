...
import { BookOpen, PlusCircle, Clock, CheckCircle, XCircle, Calendar, ChevronRight } from 'lucide-react'
import { Card, CardContent, Badge, Button, StatCard, Input, Select, Textarea } from '../../components/ui'
import { mockWorkshops, departments, aiTopics } from '../../data/mockData'
...
export default function ProfessorDashboard() {
  ...
  const myWorkshops = mockWorkshops.filter(w => w.professor.id === 'prof1')
    ...
    <div className="max-w-2xl">
      ...
      <Card><CardContent className="pt-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          <Input label="Workshop Title *" placeholder="e.g. Introduction to Neural Networks" value={form.title} onChange={set('title')}/>
          <Textarea label="Description *" placeholder="Describe the workshop content, objectives, and outcomes…" value={form.description} onChange={set('description')} rows={4}/>
          <div className="grid sm:grid-cols-2 gap-4">
            <Select label="Department *" value={form.department} onChange={set('department')} options={[{value:'',label:'Select department…'},...departments.map(d => ({value:d,label:d}))]}/>
            <Select label="AI Topic *" value={form.topic} onChange={set('topic')} options={[{value:'',label:'Select topic…'},...aiTopics.map(t => ({value:t,label:t}))]}/>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            <Input label="Date *" type="date" value={form.date} onChange={set('date')}/>
            <Input label="Time *" type="time" value={form.time} onChange={set('time')}/>
          </div>
          <Input label="Location *" placeholder="e.g. Amphithéâtre A, Bâtiment Informatique" value={form.location} onChange={set('location')}/>
          <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="flex gap-3 pt-2">
            <Button type="submit" className="flex-1">Submit for Review</Button>
