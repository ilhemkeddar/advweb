...
import { ArrowLeft, Calendar, Clock, MapPin, Users, FileText, Video, Code, SlidersHorizontal, CheckCircle } from 'lucide-react'
...
export default function WorkshopDetails() {
  ...
  const isFull = w.enrolled >= w.capacity
  const fillPct = Math.min(100,(w.enrolled/w.capacity)*100)
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex flex-wrap gap-2 mb-4">
                <Badge variant={w.status==='upcoming'?'success':'outline'}>{w.status.charAt(0).toUpperCase()+w.status.slice(1)}</Badge>
                <Badge variant="outline">{w.type==='webinar'?'🖥 Webinar':'📍 In-Person'}</Badge>
                <Badge variant="info">{w.department}</Badge>
              </div>
              <h1 className="font-display text-3xl text-[#0F1419] mb-3 leading-tight">{w.title}</h1>
              <p className="text-[#64748B] leading-relaxed">{w.description}</p>
              {w.tags && w.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-5">{w.tags.map(t => <span key={t} className="text-xs px-3 py-1 bg-[#F0F4F8] text-[#64748B] rounded-full">{t}</span>)}</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                {[
                  {icon:Calendar,label:'Date',value:new Date(w.date).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'})},
                  {icon:Clock,label:'Time',value:w.time},
                  {icon:MapPin,label:'Location',value:w.location},
                  {icon:Users,label:'Target Audience',value:w.targetAudience||'All students'},
            ...
            <CardContent className="pt-6">
              ...
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#0A5F7F] to-[#06B6D4] flex items-center justify-center text-white font-semibold text-lg">
                  {w.professor.name.split(' ').map(n => n[0]).join('').slice(0,2)}
                </div>
                <div>
                  <p className="font-semibold text-[#0F1419]">{w.professor.name}</p>
                  <p className="text-sm text-[#64748B]">{w.professor.title}</p>
                  <p className="text-sm text-[#0A5F7F]">{w.department}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <div className="space-y-3">
                  {w.resources.map((r,i) => (
                    <a key={i} href={r.url} className="flex items-center gap-3 p-3 rounded-xl border border-[rgba(15,20,25,0.08)] hover:border-[#0A5F7F]/30 hover:bg-[#F0F4F8]/50 transition-all group">
                      <div className="w-9 h-9 rounded-lg bg-[#0A5F7F]/10 flex items-center justify-center text-[#0A5F7F] group-hover:bg-[#0A5F7F] group-hover:text-white transition-all">{rIcon(r.type)}</div>
                      <div className="flex-1 min-w-0"><p className="text-sm font-medium text-[#0F1419] truncate">{r.name}</p><p className="text-xs text-[#64748B] capitalize">{r.type}</p></div>
                    </a>
                  ))}
                </div>
              </CardContent>
          ...
          <Card className="sticky top-4">
            <CardContent className="pt-6">
              ...
              {w.status === 'upcoming' ? (
                <>
                  <div className="mb-4">
                    <div className="flex justify-between text-sm mb-1.5"><span className="text-[#64748B]">Seats available</span><span className="font-medium text-[#0F1419]">{w.enrolled}/{w.capacity}</span></div>
                  ...
                  {registered ? (
                    ...
                    <Button className="w-full" disabled={isFull} onClick={() => setRegistered(true)}>{isFull?'Workshop Full':'Register Now'}</Button>
                  )}
                </>
              ) : (
                <div className="text-center py-4"><Badge variant="outline" className="mb-2">Workshop Ended</Badge><p className="text-sm text-[#64748B]">This workshop has already taken place.</p></div>
              )}
              <div className="mt-5 pt-5 border-t border-[rgba(15,20,25,0.06)] space-y-3 text-sm">
                {[{k:'Topic',v:w.topic},{k:'Format',v:w.type.replace('-',' ')},{k:'Department',v:w.department}].map(item => (
