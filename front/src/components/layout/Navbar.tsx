...
import { GraduationCap, LogIn, Menu, X, Mail, BookOpen, Users, Library } from 'lucide-react'
...
const navLinks = [
  ...
  {path: '/training', label: 'Training Hub', icon: BookOpen},
  {path: '/representatives', label: 'AI Representatives', icon: Users},
        ...
        {open && (
          <div className="md:hidden pb-4 pt-3 border-t border-[rgba(15,20,25,0.06)] space-y-1">
            {navLinks.map(l => {
              const Icon = l.icon
              return (
                <Link key={l.path} to={l.path} onClick={() => setOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-[#64748B] hover:bg-[#F0F4F8] rounded-lg">
                  <Icon className="w-4 h-4" />
                  {l.label}
                </Link>
              )
