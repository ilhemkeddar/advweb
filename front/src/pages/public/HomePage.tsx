...
import { ArrowRight, Sparkles, BookOpen, Users, Award, ChevronRight, Zap, Shield, Globe, Loader2 } from 'lucide-react'
...
export default function HomePage() {
  ...
  async function loadWorkshops() {
    try {
      ...
      // Filter for approved/upcoming workshops and take first 3
      const featured = (data as Workshop[])
        .filter(w => w.status === 'approved' || w.status === 'upcoming')
        .slice(0, 3)
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading ? (
              ...
              <div className="col-span-full text-center py-12 text-red-600">
                <p>{error}</p>
                <button onClick={loadWorkshops} className="mt-3 text-sm text-[#0A5F7F] hover:underline">Retry</button>
              </div>
            ) : workshops.length === 0 ? (
              <div className="col-span-full text-center py-12 text-[#64748B]">
                <BookOpen className="w-12 h-12 mx-auto mb-4 opacity-30"/>
                <p className="font-display text-lg">No upcoming workshops</p>
                <p className="text-sm mt-1">Check back later for new sessions!</p>
              </div>
            ) : (
              workshops.map(w => (
                <Card key={w._id} hover={true} className="flex flex-col">
                  <CardContent className="pt-6 flex flex-col h-full">
                    ...
                    <div className="space-y-1.5 text-sm text-[#64748B] mb-5">
                      ...
                      <div><span className="font-medium text-[#0F1419]">By:</span> {w.professor?.name || 'TBA'}</div>
                      <div><span className="font-medium text-[#0F1419]">Seats:</span> {w.registeredCount}/{w.capacity}</div>
                    </div>
