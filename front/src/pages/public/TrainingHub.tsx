...
export default function TrainingHub() {
  ...
  const filtered = workshops.filter(w => {
    ...
    const matchSearch = !q || w.title.toLowerCase().includes(q) || (w.professor?.name || '').toLowerCase().includes(q)
  ...
  const departments = [...new Set(workshops.filter(w => w.department).map(w => w.department))]
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="bg-white rounded-2xl border border-[rgba(15,20,25,0.08)] p-5 mb-8 shadow-sm">
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[200px]">
            <Input placeholder="Search workshops, professors…" value={search} onChange={e => setSearch(e.target.value)} leftIcon={<Search className="w-4 h-4"/>}/>
          </div>
          <div className="w-36">
            <Select value={status} onChange={e => setStatus(e.target.value)} options={[{value:'all',label:'All Status'},{value:'approved',label:'Approved'},{value:'pending',label:'Pending'}]}/>
          </div>
          <div className="w-36">
      ...
      {loading ? (
        <div className="text-center py-24">
          ...
          <p className="text-[#64748B]">Loading workshops…</p>
        </div>
      ) : filtered.length === 0 ? (
        ...
        <>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map(w => (
              <Card key={w._id} hover className="flex flex-col">
                <CardContent className="pt-6 flex flex-col h-full">
                  ...
                  <div className="space-y-1 text-sm text-[#64748B] mb-5 border-t border-[rgba(15,20,25,0.06)] pt-4">
                    ...
                    <div><span className="font-medium text-[#0F1419]">By:</span> {w.professor?.name || 'Unknown'}</div>
                    <div><span className="font-medium text-[#0F1419]">Dept:</span> {w.department}</div>
                    {w.status === 'approved' && (
                      <div>
                        <span className="font-medium text-[#0F1419]">Seats:</span> {w.registeredCount}/{w.capacity}
                        <div className="w-full h-1.5 bg-[#F0F4F8] rounded-full mt-1 overflow-hidden">
                          <div className="h-full rounded-full bg-gradient-to-r from-[#0A5F7F] to-[#06B6D4]" style={{width:`${Math.min(100,(w.registeredCount/w.capacity)*100)}%`}}/>
                        </div>
                      </div>
                    )}
                  </div>
                  <Link to={`/workshop/${w._id}`} className="block w-full text-center py-2.5 border border-[rgba(15,20,25,0.15)] rounded-xl text-sm font-medium hover:bg-[#F0F4F8] transition-colors">View Details</Link>
                </CardContent>
              </Card>
            ))}
          </div>
